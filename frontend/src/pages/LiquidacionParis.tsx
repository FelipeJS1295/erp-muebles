...
export default function LiquidacionParis() {
  ...
  const cargar = async () => {
    setLoading(true)
    try {
      const res = await api.get('/liquidaciones?marketplace=paris_chile')
    ...
    for (const l of liquidaciones) {
      ...
      if (!map[nombre]) {
        map[nombre] = {
          ...
          devoluciones: 0, despachos: 0, neto: 0,
      ...
      if (l.tipo === 'despacho')       { r.despachos += m }
    ...
    <div style={{ padding: '24px', maxWidth: '1200px', margin: '0 auto' }}>
      ...
      {loading ? (
        ...
        <div style={{ padding: '64px', textAlign: 'center', background: 'var(--bg-2)', border: '0.5px solid var(--border)', borderRadius: '12px' }}>
          ...
          <div style={{ fontSize: '13px', color: 'var(--text-3)' }}>Sube un archivo Excel de liquidación de Paris para comenzar</div>
        </div>
      ) : (
          ...
          {archivos.map(a => (
            ...
            >
              ...
              <div style={{ padding: '16px 20px', display: 'flex', alignItems: 'center', gap: '16px' }}>
                {/* Icono */}
                <div style={{ width: '40px', height: '40px', borderRadius: '10px', background: '#05966918', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                ...
                <div style={{ display: 'flex', gap: '6px', flexShrink: 0 }}>
                  <button onClick={() => setArchivoDetalle(archivoDetalle === a.nombre ? null : a.nombre)} style={{
                    ...
                    border: '0.5px solid var(--accent)', background: archivoDetalle === a.nombre ? 'var(--accent)' : 'var(--bg)',
                ...
                <div style={{ borderTop: '0.5px solid var(--border)' }}>
                  ...
                  <div style={{ padding: '12px 20px', display: 'flex', gap: '8px', alignItems: 'center', background: 'var(--bg)', flexWrap: 'wrap' }}>
                    <div style={{ position: 'relative' }}>
                      ...
                      <input placeholder="Buscar suborden..." value={busqueda}
                  ...
                  <div style={{ overflowX: 'auto' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '800px' }}>
                      <thead>
                        <tr>
                          {['Fecha', 'N° Suborden', 'Descripción', 'Tipo', 'Comisión', 'Monto', 'A Pagar', 'Orden BD'].map(h => (
