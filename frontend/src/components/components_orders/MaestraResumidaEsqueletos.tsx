...
export default function MaestraResumidaEsqueletos({ ordenes, onClose }: Props) {
  ...
  useEffect(() => {
    dbApi.getAliasEsqueleteria().then(res => {
  ...
  const guardarAlias = async () => {
    ...
    try {
      ...
      await Promise.all(skusArr.map(sku => {
        ...
        return dbApi.guardarAliasEsqueleteria(sku, f?.nombre || sku, nuevoAlias.trim())
      }))
  ...
  const quitarAlias = async (sku: string) => {
    try {
      await dbApi.eliminarAliasEsqueleteria(sku)
      const newAlias = { ...alias }
      delete newAlias[sku]
      setAlias(newAlias)
  ...
  const imprimir = () => {
    ...
    if (!ventana) return
    const filasImprimir = soloResumen ? filasResumidas : filas.map(f => ({ nombre: alias[f.sku] || f.nombre, skus: [f.sku], fechas: f.fechas }))
    const totales: Record<string, number> = {}
    fechas.forEach(f => { totales[f] = 0 })
    filasImprimir.forEach(fila => fechas.forEach(f => { totales[f] = (totales[f] || 0) + (fila.fechas[f] || 0) }))
    const totalGeneral = Object.values(totales).reduce((a, b) => a + b, 0)
    ventana.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Maestra Esqueletería</title>
    <style>
    ...
    <h2>Maestra Esqueletería · ${new Date().toLocaleDateString('es-CL')}</h2>
    <table>
      <thead><tr>
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ padding: '16px 24px', borderBottom: '0.5px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            <div style={{ fontSize: '15px', fontWeight: 600, color: 'var(--text-1)' }}>Maestra Esqueletería</div>
            <div style={{ fontSize: '12px', color: 'var(--text-3)', marginTop: '2px' }}>Agrupa productos para el área de esqueletería</div>
          </div>
      ...
      {modalAlias && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: 'var(--bg-2)', borderRadius: '12px', padding: '24px', width: '420px', border: '0.5px solid var(--border)' }}>
            <div style={{ fontSize: '15px', fontWeight: 600, color: 'var(--text-1)', marginBottom: '8px' }}>Asignar nombre — Esqueletería</div>
            <div style={{ fontSize: '13px', color: 'var(--text-3)', marginBottom: '16px' }}>
              {selected.size} producto{selected.size > 1 ? 's' : ''} seleccionado{selected.size > 1 ? 's' : ''} se mostrarán como una sola línea.
            </div>
            {sugerencia && (
            ...
            <input
              ...
              placeholder="Ej: Estructura Richter"
